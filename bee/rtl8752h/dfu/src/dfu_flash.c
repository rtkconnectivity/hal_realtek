...
bool dfu_compressed_mode = false;

#define FLASH_BASE_ADDRESS           DT_REG_ADDR(DT_CHOSEN(zephyr_flash))
#define FLASH_ADDR_TO_CACHE(address) ((address) & (~FLASH_OFFSET_TO_NO_CACHE))

const struct device *flash_dev = DEVICE_DT_GET(DT_CHOSEN(zephyr_flash_controller));

bool dfu_flash_read(uint32_t addr, uint8_t *data, uint32_t byte_len)
{
    return flash_read(flash_dev, FLASH_ADDR_TO_CACHE(addr - FLASH_BASE_ADDRESS), (void *)data,
                      byte_len) == 0;
}

bool dfu_flash_write(uint32_t addr, uint8_t *data, uint32_t byte_len)
{
    return flash_write(flash_dev, FLASH_ADDR_TO_CACHE(addr - FLASH_BASE_ADDRESS), (void *)data,
                       byte_len) == 0;
}
...
{
    bool result = false;
    result = (flash_erase(flash_dev, FLASH_ADDR_TO_CACHE(addr - FLASH_BASE_ADDRESS), 0x1000) ==
              0);
...
{
    extern bool flash_rtl8752h_unlock_flash_bp_all(const struct device * dev);
    return flash_rtl8752h_unlock_flash_bp_all(flash_dev);
}
...
{
    extern void flash_rtl8752h_lock_flash_bp(const struct device * dev);
    flash_rtl8752h_lock_flash_bp(flash_dev);
}

static void dfu_process_image_sha256(HW_SHA256_CTX *ctx, uint8_t *buf, uint32_t buf_max_size,
                                     uint8_t *pdata, size_t data_len, bool is_compressed_image)
{
    uint32_t i;
    ...
    {
        dfu_flash_read((uint32_t)pdata, buf, buf_max_size);

        if (i == 0 && !is_compressed_image)
        {
    ...
    if (remain_size)
    {
        dfu_flash_read((uint32_t)pdata, buf, remain_size);
        if (loop_cnt == 0 && !is_compressed_image)
        {
            ((T_IMG_CTRL_HEADER_FORMAT *)buf)->ctrl_flag.flag_value.not_ready = 0;
        }
...
static bool dfu_check_compressed_image_sha256(T_COMPRESS_IMG_HEADER_FORMAT *p_header)
{
    ...
    dfu_process_image_sha256(&ctx, buf, SHA256_BUFFER_SIZE, pdata,
                             p_header->ctrl_header.payload_len, true);

    hw_sha256_finish(&ctx, sha256sum);

    pdata = (uint8_t *)p_header + COMPRESS_HEADER_SHA256_OFFSET;
    dfu_flash_read((uint32_t)pdata, sha256img, SHA256_LENGTH);
...
static bool dfu_check_sha256(T_IMG_HEADER_FORMAT *p_header)
{
    ...
    dfu_flash_read((uint32_t)pdata + offsetof(T_IMG_CTRL_HEADER_FORMAT, payload_len),
                   (uint8_t *)&len, 4);
    ...
    dfu_process_image_sha256(&ctx, buf, SHA256_BUFFER_SIZE, pdata, len, false);

    hw_sha256_finish(&ctx, sha256sum);

    dfu_flash_read((uint32_t)&p_header->auth.image_hash, sha256img, SHA256_LENGTH);
...
static uint32_t dfu_flash_check_blank(uint16_t image_id, uint32_t offset, uint16_t size)
{
    ...
    {
        uint32_t r_data;
        if (dfu_flash_read(start_addr, (uint8_t *)&r_data, 4))
        {
...
void dfu_set_compressed_ready(T_COMPRESS_IMG_HEADER_FORMAT *p_header)
{
    ...
    dfu_flash_read((uint32_t)&p_header->ctrl_header, image_header, DFU_HEADER_SIZE);

    ((T_IMG_CTRL_HEADER_FORMAT *)image_header)->ctrl_flag.flag_value.compressed_not_ready = 0;

    dfu_flash_write((uint32_t)&p_header->ctrl_header, image_header, DFU_HEADER_SIZE);
}
                                          ...
                                          void *p_void, uint32_t *ret)
{
    ...
    *ret = 0;
    ...
    if (offset == 0)
    {
...
#if (SUPPORT_PUBLIC_DECODE_OTA == 1)
        if (dfu_compressed_mode)
        {
            T_COMPRESS_IMG_HEADER_FORMAT *p_header =
                (T_COMPRESS_IMG_HEADER_FORMAT *)p_void;
            total_length = p_header->ctrl_header.payload_len + IMG_HEADER_SIZE;
        }
        else
        {
        ...
        {
            DFU_PRINT_ERROR2("New Image too large! total_length=%d, temp_bank_size=%d",
                             total_length, temp_bank_size);
            *ret = __LINE__;
            return false;
        }
    }
    ...
    {
        DFU_PRINT_ERROR3("New Image single packet too large! offset=%d, length=%d, "
                         "temp_bank_size=%d",
...
static uint32_t dfu_write(uint32_t start_addr, uint32_t length, uint8_t *p_void)
{
    ...
    DFU_PRINT_INFO2("dfu_write: start_addr=0x%x, length=%d", start_addr, length);

    for (int i = 0; i < loop_cnt; i++)
    {
        if (dfu_flash_write(start_addr + i * DFU_UPDATE_LEN, p_void + i * DFU_UPDATE_LEN,
                            DFU_UPDATE_LEN))
        {

            if (dfu_flash_read(start_addr + i * DFU_UPDATE_LEN, s_val,
                               DFU_UPDATE_LEN))
            {
                if (0 !=
                    memcmp(s_val, p_void + i * DFU_UPDATE_LEN, DFU_UPDATE_LEN))
                {
                    return __LINE__;
                }
            }
    ...
    if (remain_size)
    {
        if (dfu_flash_write(start_addr + loop_cnt * DFU_UPDATE_LEN,
                            p_void + loop_cnt * DFU_UPDATE_LEN, remain_size))
        {
            if (dfu_flash_read(start_addr + loop_cnt * DFU_UPDATE_LEN, s_val,
                               remain_size))
            {
                /* ensure p_void is ram addr */
                if (0 != memcmp(s_val, p_void + loop_cnt * DFU_UPDATE_LEN,
                                remain_size))
                {
                    return __LINE__;
                }
...
void dfu_hw_aes_decrypt_image(uint8_t *input, uint8_t *output, uint32_t length)
{
    ...
    uint32_t aes_key[8] = {0x118E601A, 0x960D4CD3, 0x807A7E70, 0x1F6763DF,
                           0xD1F610FB, 0x9A971A0C, 0x092B29E2, 0x4E46F8C5
                          };

    hw_aes_init(aes_key, NULL, AES_MODE_ECB, dfu_get_enc_setting());
    do
    {
        if ((length - offset) >= 16)
        {
...
uint32_t dfu_report_target_ic_type(uint16_t image_id, uint8_t *p_ic_type)
{
    ...
    p_header =
        (T_IMG_HEADER_FORMAT *)dfu_get_active_bank_image_addr_by_img_id((T_IMG_ID)image_id);
...
uint32_t dfu_flash_erase_sector_with_retry(uint16_t image_id, uint32_t offset)
{
    ...
    {
        unlock_flash_bp_all();
        dfu_flash_erase_sector(dfu_base_addr + offset);
        lock_flash_bp();
...
L_Return:
    DFU_PRINT_INFO2("dfu_flash_erase_with_retry: cnt=%d, result=%d", cnt, result);
    return result;
}
                    ...
                    uint32_t *p_void)
{
    ...
    DFU_PRINT_INFO3("==>dfu_update: offset=0x%x, total_offset=0x%x, length=%d", offset,
                    total_offset, length);
    ...
    /* before erase temp image or write image to flash temp, check access length depend flash
     * layout */
    ...
    if (offset == 0 && length > offsetof(T_IMG_HEADER_FORMAT, uuid))
    {
        ...
        if (dfu_compressed_mode)
        {
            T_COMPRESS_IMG_HEADER_FORMAT *p_compressed_header =
                (T_COMPRESS_IMG_HEADER_FORMAT *)p_void;
            if (IMAGE_COMPRESSED ==
                p_compressed_header->ctrl_header.ctrl_flag.flag_value.image_type)
            {
                /*access ram addr, make sure image is not ready*/
                p_compressed_header->ctrl_header.ctrl_flag.flag_value
                .compressed_not_ready = 0x1;
            }
        }
        else
        {
...
#endif
        DFU_PRINT_TRACE3(
            "<==dfu_update: New Image Header=0x%x, image_id=0x%x, dfu_base_addr=0x%x",
    ...
    {
        erase_rst = dfu_flash_erase_sector(start_addr);
    }
    else   // only cross two sector
    {
        ...
        if ((start_addr / FLASH_NOR_SECTOR_SIZE) !=
            ((start_addr + length) / FLASH_NOR_SECTOR_SIZE))
        {
            if ((start_addr + length) % FLASH_NOR_SECTOR_SIZE)
            {
                erase_rst = dfu_flash_erase_sector((start_addr + length) &
                                                   ~(FLASH_NOR_SECTOR_SIZE - 1));
            }
        }
    }
...
bool dfu_check_checksum(uint16_t image_id, uint32_t offset)
{
    ...
    {
        T_COMPRESS_IMG_HEADER_FORMAT *p_compressed_header =
            (T_COMPRESS_IMG_HEADER_FORMAT *)base_addr;
        if (IMAGE_COMPRESSED ==
            p_compressed_header->ctrl_header.ctrl_flag.flag_value.image_type)
        {
            check_result = check_compressed_image_chksum(p_compressed_header);
        }
        else
        {
            error_code = __LINE__;
...
void dfu_set_image_ready(T_IMG_HEADER_FORMAT *p_header, bool compressed_mode)
{
    ...
    if (compressed_mode)
    {
        ...
        {
            DFU_PRINT_INFO1("dfu_set_image_ready: image_type %d error!",
                            p_header->ctrl_header.ctrl_flag.flag_value.image_type);
            return;
        }
    }
    else
...
bool dfu_copy_img(uint16_t image_id, uint32_t dlAddress, uint32_t dlSize)
{
    ...
    if ((image_id < AppData1) || (image_id > AppConfigFile))
    {
        error_code = __LINE__;
        goto L_Return;
    }
    ...
    while (remain_size > 0)
    {
        ...
        {
            dfu_flash_erase_sector(dest_base_addr + tmp_offset);
        }
        if (remain_size > 2048)
        {
            memcpy(p_ota_temp_buffer_head, (uint8_t *)(source_base_addr + tmp_offset),
                   2048);
            if (remain_size == dlSize)
            {
                T_IMG_CTRL_HEADER_FORMAT *p_header =
                    (T_IMG_CTRL_HEADER_FORMAT *)p_ota_temp_buffer_head;
                p_header->ctrl_flag.flag_value.not_ready =
                    0x1; /*make sure image is not ready, will use it later*/
            }
            for (int i = 0; i < 2048; i = i + 4)
            {
                dfu_flash_write(dest_base_addr + tmp_offset + i,
                                p_ota_temp_buffer_head, 4);
                dfu_flash_read((dest_base_addr + tmp_offset + i), (uint8_t *)&s_val,
                               4);
                if (s_val != *(uint32_t *)p_ota_temp_buffer_head)
                {
                    DFU_PRINT_TRACE3("s_val:0x%08x, *p_void:0x%08x, i:0x%08x",
                                     s_val, *(uint32_t *)p_ota_temp_buffer_head,
                                     i);
                    error_code = __LINE__;
                    goto L_Return;
                }
