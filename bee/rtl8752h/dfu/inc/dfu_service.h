...
#define START_DFU_COMPRESSED_MODE_VALUE         4
...
typedef struct
{
    ...
    bool is_need_dfu;
    uint8_t start_dfu_mode;
} T_DFU_PARA;

typedef struct
{
    T_IMG_ID image_id;
    bool dfu_compressed_mode;
    uint32_t image_size;
...
extern void dfu_notify_conn_para_update_req(struct bt_conn *conn, T_DFU_ARV_ERROR_CODE error_code);
extern void dfu_service_handle_valid_fw(struct bt_conn *conn);
