...
#include "pinmux/inc/rtl_pinmux.h"
#include "rtl_aon_reg.h"

/*============================================================================*
 *                           Private Functions
 *============================================================================*/
static void Pad_WakeupConfig(uint8_t Pin_Num, PAD_WAKEUP_EN Status);
static void Pad_SetWakeupPolarity(uint8_t Pin_Num, PAD_WAKEUP_POL Wakeup_Polarity);
static void Pad_WakeupDebounceCmd(uint8_t Pin_Num, PAD_WAKEUP_DEBOUNCE_EN Status);
...
/**
  ...
  * \param  Pin_Num: Pin number to be configured. \ref Pin_Number
  * \param  Pin_Func: IO function, please refer to rtl876x_pinmux.h "Pin_Function_Number" part.
  * \return None
  */
...
/**
  * \brief  Configure the specified pin to its corresponding AON IO function.
  * \param  Pin_Func: AON IO function, please refer to rtl876x_pinmux.h "PAD_Aon_Mux" part.
  * \return None
  */
...
/**
  * \brief  Configure the specified pin to its corresponding HS IO function.
  * \param  Pin_Func: HS IO function, please refer to rtl876x_pinmux.h "PAD_HS_Mux" part.
  * \return None
  */
                ...
                PAD_OUTPUT_VAL          AON_PAD_O)
{
    ...
    {
        /* RSVD  GATE  CLKP  CLKP  0011
           AMUX  DEB   ASEL  SHDN  1100
           DCUR  WKP   WKE   PULR  1111
           PULD  PULE  OUTE  OUTV  0000
         Clear reg value first. */
        reg_value &= 0xFFFF3CF0;
 ...
 * \return None.
 */
void System_WakeUpDebounceCmd(uint8_t Pin_Num, PAD_WAKEUP_EN Status)
{
    uint32_t reg_value = 0;
    uint32_t reg_addr = PAD_DEBOUNCE_WKUP_STS;
    reg_value = pad_aon_read(reg_addr);
 ...
 * \return None.
 */
void Pad_OutputCmd(uint8_t Pin_Num, PAD_OUTPUT_ENABLE_Mode Status)
{
    uint32_t reg_value = 0;
    uint32_t reg_value_high_low = Pin_Num % 2;
    uint32_t reg_addr = PAD_REG_BASE + PINADDR_TABLE[Pin_Num / 2];
 ...
 * \return None.
 */
void Pad_SetPullStrength(uint8_t Pin_Num, PAD_Pull_Strength_Mode PAD_Pull_Strength_Mode)
{
    uint32_t reg_value = 0;
    uint32_t reg_value_high_low = Pin_Num % 2;
    uint32_t reg_addr = PAD_REG_BASE + PINADDR_TABLE[Pin_Num / 2];
 ...
 * \return None.
 */
void Pad_PowerCmd(uint8_t Pin_Num, PAD_PWR_Mode PWR_Mode)
{
    uint32_t reg_value = 0;
    uint32_t reg_value_high_low = Pin_Num % 2;
    uint32_t reg_addr = PAD_REG_BASE + PINADDR_TABLE[Pin_Num / 2];
