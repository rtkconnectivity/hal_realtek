...
typedef struct
{
    ...
    uint32_t I2C_TxThresholdLevel; /* !< Specifies the transmit FIFO Threshold to trigger interrupt
                                      I2C_INT_TX_EMPTY. This parameter can be a value less than 24*/
    uint32_t I2C_RxThresholdLevel; /* !<Specifies the receive FIFO Threshold to trigger interrupt
                                      I2C_INT_RX_FULL. This parameter can be a value less than 16*/
    uint16_t I2C_TxDmaEn;          /*!< Specifies the Tx dma mode.
                                           This parameter can be a value of ENABLE or DISABLE. */
    uint16_t I2C_RxDmaEn;          /*!< Specifies the Rx dma mode.
                                           This parameter can be a value of ENABLE or DISABLE. */
    uint8_t
    I2C_TxWaterlevel;     /*!< Specifies the DMA tx water level.
...
#define IS_I2C_GET_FLAG(FLAG)                                                                      \
    (((FLAG) == I2C_FLAG_SLV_ACTIVITY) || ((FLAG) == I2C_FLAG_MST_ACTIVITY) ||                     \
     ((FLAG) == I2C_FLAG_RFF) || ((FLAG) == I2C_FLAG_RFNE) || ((FLAG) == I2C_FLAG_TFE) ||          \
     ((FLAG) == I2C_FLAG_TFNF) || ((FLAG) == I2C_FLAG_ACTIVITY))
/**
...
#define MS_ALL_ABORT                                                                               \
    (ARB_LOST | ABRT_MASTER_DIS | ABRT_TXDATA_NOACK | ABRT_10ADDR2_NOACK | ABRT_10ADDR1_NOACK |    \
     ABRT_7B_ADDR_NOACK)

#define IS_I2C_EVENT(EVENT)                                                                        \
    (((EVENT) == ABRT_SLVRD_INTX) || ((EVENT) == ABRT_SLV_ARBLOST) ||                              \
     ((EVENT) == ABRT_SLVFLUSH_TXFIFO) || ((EVENT) == ARB_LOST) || ((EVENT) == ABRT_MASTER_DIS) || \
     ((EVENT) == ABRT_10B_RD_NORSTRT) || ((EVENT) == ABRT_SBYTE_NORSTRT) ||                        \
     ((EVENT) == ABRT_HS_NORSTRT) || ((EVENT) == ABRT_SBYTE_ACKDET) ||                             \
     ((EVENT) == ABRT_HS_ACKDET) || ((EVENT) == ABRT_GCALL_READ) ||                                \
     ((EVENT) == ABRT_GCALL_NOACK) || ((EVENT) == ABRT_TXDATA_NOACK) ||                            \
     ((EVENT) == ABRT_10ADDR2_NOACK) || ((EVENT) == ABRT_10ADDR1_NOACK) ||                         \
...
#define I2C_GET_INT(INT)                                                                           \
    (((INT) == I2C_INT_GEN_CALL) || ((INT) == I2C_INT_START_DET) || ((INT) == I2C_INT_STOP_DET) || \
     ((INT) == I2C_INT_ACTIVITY) || ((INT) == I2C_INT_RX_DONE) || ((INT) == I2C_INT_TX_ABRT) ||    \
     ((INT) == I2C_INT_RD_REQ) || ((INT) == I2C_INT_TX_EMPTY) || ((INT) == I2C_INT_TX_OVER) ||     \
     ((INT) == I2C_INT_RX_FULL) || ((INT) == I2C_INT_RX_OVER) || ((INT) == I2C_INT_RX_UNDER))
 ...
 *      This parameter can be one of the following values:
 *      \arg ABRT_SLVRD_INTX: When the processor side responds to a slave mode request for data to
 * be transmitted to a remote master and user send read command. \arg ABRT_SLV_ARBLOST: Slave lost
 * the bus while transmitting data to a remote master. \arg ABRT_SLVFLUSH_TXFIFO: Slave has received
 * a read command and some data exists in the TX FIFO so the slave issues a TX_ABRT interrupt to
 * flush old data in TX FIFO. \arg ARB_LOST: Master has lost arbitration or the slave transmitter
 * has lost arbitration. \arg ABRT_MASTER_DIS: User tries to initiate a Master operation with the
 * Master mode disabled \arg ABRT_10B_RD_NORSTRT: The restart is disabled and the master sends a
 * read command in 10-bit addressing mode. \arg ABRT_SBYTE_NORSTRT: The restart is disabled and the
 * user is trying to send a START Byte. \arg ABRT_HS_NORSTRT: The restart is disabled and the user
 * is trying to use the master to transfer data in High Speed mode. \arg ABRT_SBYTE_ACKDET: Master
 * has sent a START Byte and the START Byte was acknowledged (wrong behavior). \arg ABRT_HS_ACKDET:
 * Master is in High Speed mode and the High Speed Master code was acknowledged (wrong behavior).
 *      \arg ABRT_GCALL_READ: Sent a General Call but the user programmed the byte following the
 * General Call to be a read from the bus. \arg ABRT_GCALL_NOACK: Sent a General Call and no slave
 * on the bus acknowledged the General Call. \arg ABRT_TXDATA_NOACK: Master sent data byte(s)
 * following the address, it did not receive an acknowledge from the remote slave. \arg
 * ABRT_10ADDR2_NOACK: Master is in 10-bit address mode and the second address byte of the 10-bit
 * address was not acknowledged by any slave. \arg ABRT_10ADDR1_NOACK: Master is in 10-bit address
 * mode and the first 10-bit address byte was not acknowledged by any slave. \arg
 * ABRT_7B_ADDR_NOACK: Master is in 7-bit addressing mode and th address sent was not acknowledged
 * by any slave. \return  An ErrorStatus enumeration value. \retval  SUCCESS: Last event is equal to
 * the I2C_EVENT. \retval  ERROR: Last event is different from the I2C_EVENT.
 *
 * <b>Example usage</b>
 * \code{.c}
 *
 ...
 * \endcode
 */
__STATIC_INLINE void I2C_GDMACmd(I2C_TypeDef *I2Cx, uint16_t I2C_GDMAReq,
