...
void IR_DLPS_Enter(void *PeriReg, void *StoreBuf)
{
    ...
    store_buf[0] = IRx->CLK_DIV;
    store_buf[1] = IRx->TX_CONFIG;
    store_buf[2] = IRx->RX_CONFIG;
    store_buf[3] = IRx->RX_CNT_INT_SEL;
...
void IR_DLPS_Exit(void *PeriReg, void *StoreBuf)
{
    ...
    IRx->CLK_DIV = store_buf[0];
    if (store_buf[1] & BIT31)
    {
        /* RX MODE */
        IRx->TX_CONFIG  = store_buf[1];
        IRx->RX_CONFIG  = store_buf[2];
        IRx->RX_CNT_INT_SEL  = store_buf[3];
    }
    else
    {
        /* TX MODE */
