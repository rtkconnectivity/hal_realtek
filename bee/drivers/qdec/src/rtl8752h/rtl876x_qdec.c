...
void QDEC_Init(QDEC_TypeDef *QDECx, QDEC_InitTypeDef *QDEC_InitStruct)
{
    ...
    {
        /* Clear REG_CR_X Register*/
        QDECx->REG_CR_X &= ~((0xFF << 4) | BIT13 | BIT12 | (0x3));

        /* Set Manual Initial Phase*/
        if (QDEC_InitStruct->manualLoadInitPhase == ENABLE)
        {
            QDECx->REG_CR_X &= ~BIT31;
            QDECx->REG_CR_X |= BIT2 | (QDEC_InitStruct->initPhaseX);
            QDECx->REG_CR_X &= ~BIT2;
        ...
        QDECx->REG_CR_X |= ((QDEC_InitStruct->debounceTimeX << 4) | (QDEC_InitStruct->counterScaleX << 13) |
                            \
                            (QDEC_InitStruct->debounceEnableX << 12));
    }
    if (QDEC_InitStruct->axisConfigY == ENABLE)
    {
        /* Clear REG_CR_Y Register*/
        QDECx->REG_CR_Y &= ~((0xFF << 4) | BIT13 | BIT12 | (0x3));

        /* Set Manual Initial Phase*/
        if (QDEC_InitStruct->manualLoadInitPhase == ENABLE)
        {
            QDECx->REG_CR_Y &= ~BIT31;
            QDECx->REG_CR_Y |= BIT2 | (QDEC_InitStruct->initPhaseY);
            QDECx->REG_CR_Y &= ~BIT2;
        ...
        QDECx->REG_CR_Y |= ((QDEC_InitStruct->debounceTimeY << 4) | (QDEC_InitStruct->counterScaleY << 13) |
                            \
                            (QDEC_InitStruct->debounceEnableY << 12));
    }
    if (QDEC_InitStruct->axisConfigZ == ENABLE)
    {
        /* Clear REG_CR_Z Register*/
        QDECx->REG_CR_Z &= ~((0xFF << 4) | BIT13 | BIT12 | (0x3));

        /* Set Manual Initial Phase*/
        if (QDEC_InitStruct->manualLoadInitPhase == ENABLE)
        {
            QDECx->REG_CR_Z &= ~BIT31;
            QDECx->REG_CR_Z |= BIT2 | (QDEC_InitStruct->initPhaseZ);
            QDECx->REG_CR_Z &= ~BIT2;
        ...
        QDECx->REG_CR_Z |= ((QDEC_InitStruct->debounceTimeZ << 4) | (QDEC_InitStruct->counterScaleZ << 13) |
                            \
                            (QDEC_InitStruct->debounceEnableZ << 12));
    }
    return;
}
...
void QDEC_INTConfig(QDEC_TypeDef *QDECx, uint32_t QDEC_IT, FunctionalState newState)
{
    ...
    if (newState == ENABLE)
    {
        ...
        if (QDEC_IT & QDEC_X_INT_ILLEGAL)
        {
            QDECx->REG_CR_X |= BIT14;
        }
        if (QDEC_IT & QDEC_Y_INT_ILLEGAL)
        {
            QDECx->REG_CR_Y |= BIT14;
        }
        if (QDEC_IT & QDEC_Z_INT_ILLEGAL)
        {
              ...
              FunctionalState newState)
{
    ...
    if (newState == ENABLE)
    {
        *((volatile uint32_t *)(&QDECx->REG_CR_X) + QDEC_AXIS / 2) |= BIT31;
    }
    else
    {
        *((volatile uint32_t *)(&QDECx->REG_CR_X) + QDEC_AXIS / 2) &= ~(BIT31);
